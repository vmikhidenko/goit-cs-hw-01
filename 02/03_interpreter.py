...
class TokenType:
    ...
    PLUS = "PLUS"
    MINUS = "MINUS"
    MUL = "MUL"         # Новий токен для множення
    DIV = "DIV"         # Новий токен для ділення
    LPAREN = "LPAREN"   # Новий токен для '('
    RPAREN = "RPAREN"   # Новий токен для ')'
...
class Lexer:
    # Код лексера такий самий, як у 01_lexer.py
    ...
    def get_next_token(self):
        ...
        while self.current_char is not None:
            ...
            raise LexicalError(f"Помилка лексичного аналізу на позиції {self.pos}: невідомий символ '{self.current_char}'")
...
class Parser:
    # Код парсера такий самий, як у 02_parser.py
    def __init__(self, lexer):
        self.lexer = lexer
        self.current_token = self.lexer.get_next_token()

    def error(self):
        raise ParsingError(f"Помилка синтаксичного аналізу на позиції {self.lexer.pos}")
    ...
    def factor(self):
        """Парсер для 'factor': INTEGER або '(' expr ')'."""
    ...
    def term(self):
        """Парсер для 'term': factor ((MUL|DIV) factor)*."""
        node = self.factor()
    ...
    def expr(self):
        """Парсер для 'expr': term ((PLUS|MINUS) term)*."""
        node = self.term()
...
class Interpreter:
    ...
    def visit_BinOp(self, node):
        if node.op.type == TokenType.PLUS:
            ...
            raise Exception(f"Неочікуваний оператор {node.op.type}")
...
# Введення: 7 + 3 * (10 / (12 / (3 + 1) - 1))
# Очікуваний результат: 22.0
# Введіть вираз (або "exit" для виходу): 7 + 3 * (10 / (12 / (3 + 1) - 1))
# 22.0
